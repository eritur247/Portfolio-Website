...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function generateColor() {
      // Restrict hue to blues, purples, and pinks only (avoid green/yellow/cyan)
      // Hue range: 0.58-0.95 (blue -> purple -> magenta -> pink)
      const hue = 0.58 + Math.random() * 0.37;
    ...
    function getCanvasCoordinates(e) {
      // Since canvas is fixed position, use clientX/Y directly
      return {
    ...
    function handleFirstMouseMove(e) {
      ...
      const color = generateColor();
      updateFrame();
      updatePointerMoveData(pointer, coords.x, coords.y, color);
    ...
    function getTouchCoordinates(touch) {
      // Since canvas is fixed position, use clientX/Y directly
      return {
        x: scaleByPixelRatio(touch.clientX),
        y: scaleByPixelRatio(touch.clientY)
      };
    }
    ...
    function handleFirstTouchStart(e) {
      ...
      for (let i = 0; i < touches.length; i++) {
        const coords = getTouchCoordinates(touches[i]);
        updateFrame();
